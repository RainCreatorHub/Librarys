...
function MoonLibV2:MakeWindow(WindowInfo)
    ...
    if WindowInfo.LoadScreen then
        ...
        LoadFrame.BackgroundColor3 = Theme.Primary
        ...
        LoadCorner.CornerRadius = Theme.CornerRadius
        ...
        LoadTitle.Position = UDim2.new(0.5, 0, 0, 10)
        LoadTitle.AnchorPoint = Vector2.new(0.5, 0)
        LoadTitle.BackgroundTransparency = 1
        LoadTitle.Font = Theme.Font
        LoadTitle.TextColor3 = Theme.FontColor
        LoadTitle.TextSize = 18
        LoadTitle.Text = WindowInfo.Name or "Carregando Interface"
        
        local LoadInfoLabel = Instance.new("TextLabel")
        LoadInfoLabel.Name = "LoadInfo"
        LoadInfoLabel.Parent = LoadFrame
        LoadInfoLabel.Size = UDim2.new(1, -20, 0, 20)
        LoadInfoLabel.Position = UDim2.new(0.5, 0, 0, 40)
        LoadInfoLabel.AnchorPoint = Vector2.new(0.5, 0)
        LoadInfoLabel.BackgroundTransparency = 1
        LoadInfoLabel.Font = Enum.Font.Gotham
        LoadInfoLabel.TextColor3 = Theme.FontColorSecondary
        LoadInfoLabel.TextSize = 14
        LoadInfoLabel.Text = WindowInfo.LoadScreenInfo or "Por favor, aguarde..."
        ...
        ProgressCorner.CornerRadius = Theme.CornerRadius
        ProgressCorner.Parent = ProgressBar
        
        local ProgressTween = TweenService:Create(ProgressBar, TweenInfo.new(1.5, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 1, 0)})
        ProgressTween:Play()
    end
    ...
    task.spawn(function()
        ...
        CloseButton.TextColor3 = Theme.FontColor
        CloseButton.TextSize = 14
        CloseButton.MouseButton1Click:Connect(function()
            local tween = TweenService:Create(MainWindow, TweenInfo.new(0.3), {Size = UDim2.fromOffset(0,0), Position = UDim2.fromOffset(UserInputService:GetMouseLocation().X, UserInputService:GetMouseLocation().Y)})
            tween:Play()
        ...
        MinimizeButton.TextColor3 = Theme.FontColor
        ...
        MinimizeButton.MouseButton1Click:Connect(function()
            ...
            if IsMinimized then
                ...
                targetSize = UDim2.new(MainWindow.Size.X.Scale, MainWindow.Size.X.Offset, 0, TitleBar.AbsoluteSize.Y)
            else
                targetSize = UDim2.new(MainWindow.Size.X.Scale, MainWindow.Size.X.Offset, 0, OriginalSizeY)
            end
            
            TweenService:Create(MainWindow, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {Size = targetSize}):Play()
            ...
            for _, tab in ipairs(WindowObject.Tabs) do
                local isVisible = not IsMinimized and tab.Button.BackgroundColor3 == Theme.Accent
                local targetTransparency = isVisible and 0 or 1
                TweenService:Create(tab.Content, TweenInfo.new(0.2), {GroupTransparency = targetTransparency}):Play()
            end
        end)
    ...
    function WindowObject:MakeTab(TabInfo)
        ...
        function TabObject:MakeSubTab(SubTabInfo)
            ...
            SubTabButton.MouseButton1Click:Connect(function()
                for _, OtherSubTab in ipairs(TabObject.SubTabs) do
                    TweenService:Create(OtherSubTab.Content, TweenInfo.new(0.2), {GroupTransparency = 1}):Play()
                    OtherSubTab.Button.BackgroundColor3 = Theme.Secondary
                end
                TweenService:Create(SubTabContent, TweenInfo.new(0.2), {GroupTransparency = 0}):Play()
        ...
        local function OnTabClick()
            if WindowObject.Frame.Parent.Enabled == false or IsMinimized then return end
